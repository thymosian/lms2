...
## 2025-03-24 - [Consolidating Nested Prisma Relations in Next.js Server Components]
**Learning:** Across the `src/app/dashboard/(main)/layout.tsx`, `src/app/worker/layout.tsx`, and `src/app/dashboard/(main)/profile/page.tsx` files, there was a consistent pattern of executing two separate sequential `await prisma.findUnique` database calls per page load/route navigation. The first call fetched `profile` (e.g., for `fullName`) and the second fetched `user` (e.g., for `role` and `organizationId`).
**Action:** When dealing with root-level context lookups (like user profiles, user permissions, or user organizations) in Next.js server components or layouts, always combine these into a single unified `prisma.user.findUnique` query using `include: { profile: true, organization: true }` or nested `select` statements to eliminate redundant database network requests.
